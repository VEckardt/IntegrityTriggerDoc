...
public class APISession {
    ...
    // Log file location
    public static String LOGFILE = tmpDir + fs + "IntegrityAddField_" + getDate() + ".log";
     ...
     *
     * @param hostname MKS Integrity Server Name
     * @param port MKS Integrity Server Port
     * @param secure Boolean to toggle whether or not SSL is in use
     * @param user MKS Integrity User name
     * @param password MKS Integrity Password
     * @throws APIException
     */
    ...
    public WorkItem getTypeRole(String typeName) {
        Command cmd = new Command("im", "viewtype");
        ...
        try {
            Response response = cmdRunner.execute(cmd);
            return response.getWorkItem(typeName);
        } catch (APIException ex) {
            logger.exception(APISession.class.getName(), MKSLogger.ERROR, 1, ex);
        }
        return null;
